#!/usr/bin/env python3
"""
Script to list all products from farm_prices table
...
def get_farm_prices_products():
    ...
    print("Fetching all products from remote Supabase farm_prices table...")
    print("=" * 60)
...
def show_products_summary(products_df):
    """Show summary of farm prices products"""
    
    print(f"\n📋 FARM PRICES PRODUCTS SUMMARY")
    print("=" * 50)
...
def create_local_table(products_df):
    """Create a local table with farm prices products"""
    
    print("\n💾 Creating local table with farm prices products...")
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            drop_query = "DROP TABLE IF EXISTS public.farm_prices CASCADE"
            conn.execute(text(drop_query))
            conn.commit()
            print("  🗑️ Removed existing farm_prices table if it existed")
            
            # Create new table
            create_query = """
            CREATE TABLE public.farm_prices (
                id TEXT PRIMARY KEY,
            ...
            conn.commit()
            print("  ✅ Created farm_prices table")
            ...
            for i in range(0, total_rows, chunk_size):
                ...
                for _, row in chunk_df.iterrows():
                    try:
                        insert_query = text("""
                            INSERT INTO public.farm_prices 
            ...
            # Verify insertion
            verify_query = "SELECT COUNT(*) as count FROM public.farm_prices"
...
def export_to_csv(products_df):
    ...
    try:
        csv_filename = "farm_prices_products.csv"
        ...
        unique_csv_filename = "farm_prices_unique_products.csv"
...
def main():
    """Main function"""
    print("Listing All Products from Farm Prices Table")
    print("=" * 60)
    
    try:
        # Step 1: Get all products from remote Supabase farm_prices table
        ...
        print(f"\n🎯 FARM PRICES PRODUCTS LISTING COMPLETED!")
        print(f"📁 Local database: public.farm_prices table created")
        print(f"📄 CSV files created:")
        print(f"  - farm_prices_products.csv")
        print(f"  - farm_prices_unique_products.csv")
