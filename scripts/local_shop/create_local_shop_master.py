...
def get_local_shop_products():
    ...
    print("📥 Fetching products from remote Supabase local_shop_prices table...")
    print("=" * 70)
...
def create_local_shop_master_table(products_df):
    """Create master table for local shop products using standardization logic"""
...
def write_master_table_to_db(master_df):
    ...
    print("\n--- Writing Local Shop Master Table to Local Database ---")
    local_engine = get_db_engine('hub')
    table_name = 'local_shop_master'
    schema = 'public'
...
def create_verification_table(products_df, master_df):
    """Create verification table for local shop products"""
    
    print("\n--- Creating Local Shop Verification Table ---")
    ...
    verification_df.rename(columns={
        ...
        'raw_product_name': 'all_local_shop_products'
    }, inplace=True)

    # Ensure unique product names in the verification table
    verification_df = verification_df.drop_duplicates(subset=['all_local_shop_products'])
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            CREATE TABLE public.local_shop_verification (
                ...
                all_local_shop_products TEXT NOT NULL,
                parent_name TEXT,
...
def main():
    """Main function"""
    print("Local Shop Master Table Creation")
    print("=" * 50)
    print("📥 Source: Remote Supabase local_shop_prices table")
    ...
    try:
        ...
        print(f"\n🎯 LOCAL SHOP MASTER TABLE CREATED!")
        print(f"📁 Local database: public.local_shop_master table created")
        print(f"📁 Local database: public.local_shop_verification table created")
        print(f"📄 CSV file created: local_shop_verification.csv")
