#!/usr/bin/env python3
"""
Script to list all products from local_shop_prices table
...
def show_products_summary(products_df):
    """Show summary of local shop products"""
    
    print(f"\n📋 LOCAL SHOP PRODUCTS SUMMARY")
    print("=" * 50)
...
def create_local_table(products_df):
    """Create a local table with local shop products"""
    
    print("\n💾 Creating local table with local shop products...")
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            drop_query = "DROP TABLE IF EXISTS public.local_shop_prices CASCADE"
            conn.execute(text(drop_query))
            conn.commit()
            print("  🗑️ Removed existing local_shop_prices table if it existed")
            
            # Create new table
            create_query = """
            CREATE TABLE public.local_shop_prices (
                id TEXT PRIMARY KEY,
            ...
            conn.commit()
            print("  ✅ Created local_shop_prices table")
            ...
            for i in range(0, total_rows, chunk_size):
                ...
                for _, row in chunk_df.iterrows():
                    try:
                        insert_query = text("""
                            INSERT INTO public.local_shop_prices 
            ...
            # Verify insertion
            verify_query = "SELECT COUNT(*) as count FROM public.local_shop_prices"
...
def export_to_csv(products_df):
    ...
    try:
        csv_filename = "local_shop_products.csv"
        ...
        unique_csv_filename = "local_shop_unique_products.csv"
...
def main():
    """Main function"""
    print("Listing All Products from Local Shop Prices Table")
    print("=" * 60)
    
    try:
        # Step 1: Get all products from remote Supabase local_shop_prices table
        ...
        print(f"\n🎯 LOCAL SHOP PRODUCTS LISTING COMPLETED!")
        print(f"📁 Local database: public.local_shop_prices table created")
        print(f"📄 CSV files created:")
        print(f"  - local_shop_products.csv")
        print(f"  - local_shop_unique_products.csv")
