#!/usr/bin/env python3
"""
Add parent_product_id to REMOTE local_shop_prices table in Supabase
Using parent IDs from local_shop_master table
"""
...
def get_local_shop_master_data():
    """Get parent-child relationships from local_shop_master table"""
    
    print("Fetching parent-child relationships from local_shop_master...")
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            if df.empty:
                print("ERROR: No data found in local_shop_master table!")
                return {}
            
            print(f"SUCCESS: Found {len(df)} parent products in local_shop_master")
        ...
        print(f"ERROR: Error fetching local_shop_master data: {e}")
        return {}
...
def main():
    """Main function"""
    print("Adding parent_product_id to REMOTE local_shop_prices table in Supabase")
    print("USING PARENT IDs FROM LOCAL_SHOP_MASTER TABLE")
    print("=" * 60)
    
    try:
        # Step 1: Get parent-child relationships from local_shop_master
        ...
        add_parent_product_id_to_remote_table(supabase_engine, 'local_shop_prices', parent_mapping)
        
        # Step 4: Verify assignments
        print("\nVerification Results:")
        print("=" * 40)
        verify_parent_assignments(supabase_engine, 'local_shop_prices')
        
        print("\nSUCCESS: Parent product ID assignment completed for remote local_shop_prices!")
