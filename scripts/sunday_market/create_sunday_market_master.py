#!/usr/bin/env python3
"""
Script to create local master table for sunday_market_prices using standardization logic
...
# Parent-child mapping for sunday market products (based on standardization.py)
SUNDAY_MARKET_PARENT_CHILD_MAPPING = {
...
CHILD_TO_PARENT_MAP = _create_child_to_parent_map(SUNDAY_MARKET_PARENT_CHILD_MAPPING)
...
def get_sunday_market_products():
    ...
    print("📥 Fetching products from remote Supabase sunday_market_prices table...")
    print("=" * 70)
...
def create_sunday_market_master_table(products_df):
    """Create master table for sunday market products using standardization logic"""
    
    print("\n--- Generating Sunday Market Master Table with Parent-Child Structure ---")
    ...
    parent_groups['all_possible_child_names'] = parent_groups['parent_name'].map(SUNDAY_MARKET_PARENT_CHILD_MAPPING)
...
def write_master_table_to_db(master_df):
    ...
    print("\n--- Writing Sunday Market Master Table to Local Database ---")
    local_engine = get_db_engine('hub')
    table_name = 'sunday_market_master'
    schema = 'public'
...
def create_verification_table(products_df, master_df):
    """Create verification table for sunday market products"""
    
    print("\n--- Creating Sunday Market Verification Table ---")
    ...
    verification_df.rename(columns={
        ...
        'raw_product_name': 'all_sunday_market_products'
    }, inplace=True)

    # Ensure unique product names in the verification table
    verification_df = verification_df.drop_duplicates(subset=['all_sunday_market_products'])

    try:
        local_engine = get_db_engine('hub')
        
        with local_engine.connect() as conn:
            drop_query = "DROP TABLE IF EXISTS public.sunday_market_verification CASCADE"
            ...
            create_query = """
            CREATE TABLE public.sunday_market_verification (
                id SERIAL PRIMARY KEY,
                all_sunday_market_products TEXT NOT NULL,
                parent_name TEXT,
            ...
            conn.commit()
            print("  ✅ Created sunday_market_verification table")
            
            # Insert data into verification table
            verification_df.to_sql(
                name='sunday_market_verification',
                con=local_engine,
                schema='public',
                if_exists='append',
                index=False
...
def export_verification_to_csv(verification_df):
    ...
    try:
        csv_filename = "sunday_market_verification.csv"
...
def main():
    """Main function"""
    print("Sunday Market Master Table Creation")
    print("=" * 50)
    print("📥 Source: Remote Supabase sunday_market_prices table")
    ...
    try:
        ...
        master_df = create_sunday_market_master_table(products_df)
        if master_df.empty:
        ...
        print(f"\n🎯 SUNDAY MARKET MASTER TABLE CREATED!")
        print(f"📁 Local database: public.sunday_market_master table created")
        print(f"📁 Local database: public.sunday_market_verification table created")
        print(f"📄 CSV file created: sunday_market_verification.csv")
