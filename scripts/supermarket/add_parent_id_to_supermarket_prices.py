#!/usr/bin/env python3
"""
Script to add parent_product_id column to local supermarket_prices table
...
def get_supermarket_prices_master_data():
    """Get parent-child relationships from supermarket_prices_master table"""
    
    print("Fetching parent-child relationships from supermarket_prices_master...")
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            SELECT 
                ...
                parent_product_name,
                child_product_names
            FROM public.supermarket_master
            ORDER BY parent_product_name
            """
            df = pd.read_sql(query, conn)
            
            if df.empty:
                print("ERROR: No data found in supermarket_master table!")
                return {}
            
            print(f"SUCCESS: Found {len(df)} parent products in supermarket_master")
        ...
        print(f"ERROR: Error fetching supermarket_master data: {e}")
        return {}
...
def main():
    """Main function"""
    print("Adding parent_product_id to LOCAL supermarket_prices table")
    print("USING PARENT IDs FROM SUPERMARKET_MASTER TABLE")
    print("=" * 60)
    
    try:
        # Step 1: Get parent-child relationships from supermarket_master
        parent_mapping = get_supermarket_prices_master_data()
        ...
        # Step 3: Add parent_product_id to supermarket_prices table
        if add_parent_product_id_to_table(local_engine, 'supermarket_prices', parent_mapping):
            print("\nVerification Results:")
            print("=" * 40)
            verify_parent_product_ids(local_engine, 'supermarket_prices')
            print("\nSUCCESS: Parent product ID assignment completed for supermarket_prices!")
        else:
            print("\nERROR: Failed to add parent_product_id to supermarket_prices table.")
