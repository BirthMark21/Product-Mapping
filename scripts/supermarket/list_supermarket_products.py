#!/usr/bin/env python3
"""
Script to list all products from supermarket_prices table
...
def get_supermarket_products():
    ...
    print("Fetching all products from remote Supabase supermarket_prices table...")
    print("=" * 60)
...
def show_products_summary(products_df):
    """Show summary of supermarket products"""
    
    print(f"\n📋 SUPERMARKET PRODUCTS SUMMARY")
    print("=" * 50)
...
def create_local_table(products_df):
    """Create a local table with supermarket products"""
    
    print("\n💾 Creating local table with supermarket products...")
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            drop_query = "DROP TABLE IF EXISTS public.supermarket_prices CASCADE"
            conn.execute(text(drop_query))
            conn.commit()
            print("  🗑️ Removed existing supermarket_prices table if it existed")
            
            # Create new table
            create_query = """
            CREATE TABLE public.supermarket_prices (
                id TEXT PRIMARY KEY,
            ...
            conn.commit()
            print("  ✅ Created supermarket_prices table")
            ...
            for i in range(0, total_rows, chunk_size):
                ...
                for _, row in chunk_df.iterrows():
                    try:
                        insert_query = text("""
                            INSERT INTO public.supermarket_prices 
            ...
            # Verify insertion
            verify_query = "SELECT COUNT(*) as count FROM public.supermarket_prices"
...
def export_to_csv(products_df):
    ...
    try:
        csv_filename = "supermarket_products.csv"
        ...
        unique_csv_filename = "supermarket_unique_products.csv"
...
def main():
    """Main function"""
    print("Listing All Products from Supermarket Prices Table")
    print("=" * 60)
    
    try:
        # Step 1: Get all products from remote Supabase supermarket_prices table
        ...
        print(f"\n🎯 SUPERMARKET PRODUCTS LISTING COMPLETED!")
        print(f"📁 Local database: public.supermarket_prices table created")
        print(f"📄 CSV files created:")
        print(f"  - supermarket_products.csv")
        print(f"  - supermarket_unique_products.csv")
