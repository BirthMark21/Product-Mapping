#!/usr/bin/env python3
"""
Script to list all products from ecommerce_prices table
...
def get_ecommerce_products():
    ...
    print("Fetching all products from remote Supabase ecommerce_prices table...")
    print("=" * 60)
...
def show_products_summary(products_df):
    """Show summary of ecommerce products"""
    
    print(f"\n📋 ECOMMERCE PRODUCTS SUMMARY")
    print("=" * 50)
...
def create_local_table(products_df):
    """Create a local table with ecommerce products"""
    
    print("\n💾 Creating local table with ecommerce products...")
    ...
    try:
        ...
        with local_engine.connect() as conn:
            ...
            drop_query = "DROP TABLE IF EXISTS public.ecommerce_prices CASCADE"
            conn.execute(text(drop_query))
            conn.commit()
            print("  🗑️ Removed existing ecommerce_prices table if it existed")
            
            # Create new table
            create_query = """
            CREATE TABLE public.ecommerce_prices (
                id TEXT PRIMARY KEY,
            ...
            conn.commit()
            print("  ✅ Created ecommerce_prices table")
            ...
            for i in range(0, total_rows, chunk_size):
                ...
                for _, row in chunk_df.iterrows():
                    try:
                        insert_query = text("""
                            INSERT INTO public.ecommerce_prices 
            ...
            # Verify insertion
            verify_query = "SELECT COUNT(*) as count FROM public.ecommerce_prices"
...
def export_to_csv(products_df):
    ...
    try:
        csv_filename = "ecommerce_products.csv"
        ...
        unique_csv_filename = "ecommerce_unique_products.csv"
...
def main():
    """Main function"""
    print("Listing All Products from Ecommerce Prices Table")
    print("=" * 60)
    
    try:
        # Step 1: Get all products from remote Supabase ecommerce_prices table
        ...
        print(f"\n🎯 ECOMMERCE PRODUCTS LISTING COMPLETED!")
        print(f"📁 Local database: public.ecommerce_prices table created")
        print(f"📄 CSV files created:")
        print(f"  - ecommerce_products.csv")
        print(f"  - ecommerce_unique_products.csv")
