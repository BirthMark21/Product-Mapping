#!/usr/bin/env python3
"""
Script to create local master table for ecommerce_prices using standardization logic
...
# Parent-child mapping for ecommerce products (based on standardization.py)
ECOMMERCE_PARENT_CHILD_MAPPING = {
...
CHILD_TO_PARENT_MAP = _create_child_to_parent_map(ECOMMERCE_PARENT_CHILD_MAPPING)
...
def get_ecommerce_products():
    """Get all products from remote Supabase ecommerce_prices table"""
    
    print("📥 Fetching products from remote Supabase ecommerce_prices table...")
    print("=" * 70)
    ...
    try:
        ...
        with supabase_engine.connect() as conn:
            ...
            if df.empty:
                print("❌ No products found in remote Supabase ecommerce_prices table!")
                return pd.DataFrame()
            
            print(f"✅ Found {len(df)} products in remote Supabase ecommerce_prices table")
            
            return df
            
    except Exception as e:
        print(f"❌ Error fetching from remote Supabase ecommerce_prices: {e}")
...
def create_ecommerce_master_table(products_df):
    """Create master table for ecommerce products using standardization logic"""
    
    print("\n--- Generating Ecommerce Master Table with Parent-Child Structure ---")
    ...
    parent_groups['all_possible_child_names'] = parent_groups['parent_name'].map(ECOMMERCE_PARENT_CHILD_MAPPING)
...
def write_master_table_to_db(master_df):
    ...
    print("\n--- Writing Ecommerce Master Table to Local Database ---")
    local_engine = get_db_engine('hub')
    table_name = 'ecommerce_master'
    schema = 'public'
...
def create_verification_table(products_df, master_df):
    """Create verification table for ecommerce products"""
    
    print("\n--- Creating Ecommerce Verification Table ---")
    ...
    verification_df.rename(columns={
        ...
        'raw_product_name': 'all_ecommerce_products'
    }, inplace=True)

    # Ensure unique product names in the verification table
    verification_df = verification_df.drop_duplicates(subset=['all_ecommerce_products'])

    try:
        local_engine = get_db_engine('hub')
        
        with local_engine.connect() as conn:
            drop_query = "DROP TABLE IF EXISTS public.ecommerce_verification CASCADE"
            ...
            create_query = """
            CREATE TABLE public.ecommerce_verification (
                id SERIAL PRIMARY KEY,
                all_ecommerce_products TEXT NOT NULL,
                parent_name TEXT,
            ...
            conn.commit()
            print("  ✅ Created ecommerce_verification table")
            
            # Insert data into verification table
            verification_df.to_sql(
                name='ecommerce_verification',
                con=local_engine,
                schema='public',
                if_exists='append',
                index=False
...
def export_verification_to_csv(verification_df):
    ...
    try:
        csv_filename = "ecommerce_verification.csv"
...
def main():
    """Main function"""
    print("Ecommerce Master Table Creation")
    print("=" * 50)
    print("📥 Source: Remote Supabase ecommerce_prices table")
    print("🎯 Goal: Create local master table with standardization")
    print("=" * 50)
    
    try:
        # Step 1: Get products from remote Supabase
        products_df = get_ecommerce_products()
        if products_df.empty:
            print("No products found. Cannot proceed.")
            return
        
        # Step 2: Create master table
        master_df = create_ecommerce_master_table(products_df)
        if master_df.empty:
        ...
        print(f"\n🎯 ECOMMERCE MASTER TABLE CREATED!")
        print(f"📁 Local database: public.ecommerce_master table created")
        print(f"📁 Local database: public.ecommerce_verification table created")
        print(f"📄 CSV file created: ecommerce_verification.csv")
        ...
        print(f"🔗 Source: Remote Supabase ecommerce_prices table")
