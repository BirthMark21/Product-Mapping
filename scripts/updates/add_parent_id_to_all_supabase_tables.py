#!/usr/bin/env python3
"""
Script to add parent_product_id column to all 6 local Supabase tables
...
def remove_existing_parent_columns(engine, table_name):
    """Remove existing parent columns from a table"""
    
    print(f"  Removing existing parent columns from {table_name}...")
    
    try:
        with engine.connect() as conn:
            # Check which parent columns exist
            check_columns_query = """
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name = %s 
            AND column_name IN ('parent_id', 'parent_name', 'parent_product_id')
            AND table_schema = 'public'
            """
            existing_columns = pd.read_sql(check_columns_query, conn, params=(table_name,))
            
            if existing_columns.empty:
                print(f"    No existing parent columns found in {table_name}")
                return True
            
            # Remove each existing parent column
            columns_to_remove = existing_columns['column_name'].tolist()
            print(f"    Found existing columns: {columns_to_remove}")
            
            for column in columns_to_remove:
                try:
                    drop_column_query = f"ALTER TABLE public.{table_name} DROP COLUMN IF EXISTS {column}"
            ...
            conn.commit()
            print(f"    Successfully removed existing parent columns from {table_name}")
            return True
...
def add_parent_product_id_to_table(engine, table_name, parent_mapping):
    """Add parent_product_id column to a specific table"""
    
    print(f"\nProcessing table: {table_name}")
    
    try:
        with engine.connect() as conn:
            # Step 1: Remove existing parent columns first
            remove_success = remove_existing_parent_columns(engine, table_name)
            if not remove_success:
                print(f"  Failed to remove existing columns from {table_name}")
                return False
            
            # Step 2: Check if parent_product_id column already exists
            check_column_query = """
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name = %s 
            ...
            print(f"  ✅ Added parent_product_id column to {table_name}")
            
            # Use correct name column
            name_col = "product_name"
            if table_name == "products":
                name_col = "name"
            elif table_name == "product_variations":
...
def verify_parent_product_ids(engine, table_name):
    """Verify parent_product_id assignments in a table"""
...
def main():
    """Main function"""
    print("Adding parent_product_id to all 6 LOCAL Supabase tables")
    print("USING PARENT IDs FROM CANONICAL MASTER TABLE")
    print("(First removing existing parent columns: parent_id, parent_name, parent_product_id)")
    print("=" * 60)
    ...
    try:
        ...
        # Step 3: Get list of Supabase tables (assuming they exist locally)
        supabase_tables = settings.SUPABASE_TABLES
        print(f"Processing {len(supabase_tables)} Supabase tables:")
        for table in supabase_tables:
            print(f"  - {table}")
        
        print(f"\nCorrect table names from settings.py:")
        print(f"  - farm_prices")
        ...
        # Step 4: Add parent_product_id to each table
        successful_tables = []
        failed_tables = []
        
        for table in supabase_tables:
            success = add_parent_product_id_to_table(local_engine, table, parent_mapping)
            if success:
        ...
        for table in successful_tables:
            verify_parent_product_ids(local_engine, table)
