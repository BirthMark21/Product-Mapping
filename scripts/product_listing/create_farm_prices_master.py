#!/usr/bin/env python3
"""
Script to create local master table for farm_prices using standardization logic
...
# Parent-child mapping for farm prices products (based on standardization.py)
...
def get_farm_prices_products():
    """Get all products from remote Supabase farm_prices table"""
    
    print("📥 Fetching products from remote Supabase farm_prices table...")
    print("=" * 70)
    ...
    try:
        ...
        with supabase_engine.connect() as conn:
            ...
            if df.empty:
                print("❌ No products found in remote Supabase farm_prices table!")
                return pd.DataFrame()
            
            print(f"✅ Found {len(df)} products in remote Supabase farm_prices table")
            
            return df
            
    except Exception as e:
        print(f"❌ Error fetching from remote Supabase farm_prices: {e}")
...
def create_farm_prices_master_table(products_df):
    """Create master table for farm prices products using standardization logic"""
    
    print("\n--- Generating Farm Prices Master Table with Parent-Child Structure ---")
    ...
    parent_groups['all_possible_child_names'] = parent_groups['parent_name'].map(FARM_PRICES_PARENT_CHILD_MAPPING)
...
def write_master_table_to_db(master_df):
    ...
    print("\n--- Writing Farm Prices Master Table to Local Database ---")
    local_engine = get_db_engine('hub')
    table_name = 'farm_prices_master'
    schema = 'public'
...
def create_verification_table(products_df, master_df):
    """Create verification table for farm prices products"""
    
    print("\n--- Creating Farm Prices Verification Table ---")
    ...
    verification_df.rename(columns={
        ...
        'raw_product_name': 'all_farm_prices_products'
    }, inplace=True)

    # Ensure unique product names in the verification table
    verification_df = verification_df.drop_duplicates(subset=['all_farm_prices_products'])

    try:
        local_engine = get_db_engine('hub')
        
        with local_engine.connect() as conn:
            drop_query = "DROP TABLE IF EXISTS public.farm_prices_verification CASCADE"
            ...
            create_query = """
            CREATE TABLE public.farm_prices_verification (
                id SERIAL PRIMARY KEY,
                all_farm_prices_products TEXT NOT NULL,
                parent_name TEXT,
            ...
            conn.commit()
            print("  ✅ Created farm_prices_verification table")
            
            # Insert data into verification table
            verification_df.to_sql(
                name='farm_prices_verification',
                con=local_engine,
                schema='public',
                if_exists='append',
                index=False
...
def export_verification_to_csv(verification_df):
    ...
    try:
        csv_filename = "farm_prices_verification.csv"
...
def main():
    """Main function"""
    print("Farm Prices Master Table Creation")
    print("=" * 50)
    print("📥 Source: Remote Supabase farm_prices table")
    print("🎯 Goal: Create local master table with standardization")
    print("=" * 50)
    
    try:
        # Step 1: Get products from remote Supabase
        products_df = get_farm_prices_products()
        if products_df.empty:
            print("No products found. Cannot proceed.")
            return
        
        # Step 2: Create master table
        master_df = create_farm_prices_master_table(products_df)
        if master_df.empty:
        ...
        print(f"\n🎯 FARM PRICES MASTER TABLE CREATED!")
        print(f"📁 Local database: public.farm_prices_master table created")
        print(f"📁 Local database: public.farm_prices_verification table created")
        print(f"📄 CSV file created: farm_prices_verification.csv")
        ...
        print(f"🔗 Source: Remote Supabase farm_prices table")
