#!/usr/bin/env python3
"""
Script to create local master table for distribution_center_prices using standardization logic
...
# Parent-child mapping for distribution center products (based on standardization.py)
DISTRIBUTION_CENTER_PARENT_CHILD_MAPPING = {
...
CHILD_TO_PARENT_MAP = _create_child_to_parent_map(DISTRIBUTION_CENTER_PARENT_CHILD_MAPPING)
...
def create_distribution_center_master_table(products_df):
    """Create master table for distribution center products using standardization logic"""
    
    print("\n--- Generating Distribution Center Master Table with Parent-Child Structure ---")
    ...
    parent_groups['all_possible_child_names'] = parent_groups['parent_name'].map(DISTRIBUTION_CENTER_PARENT_CHILD_MAPPING)
...
def write_master_table_to_db(master_df):
    ...
    print("\n--- Writing Distribution Center Master Table to Local Database ---")
    local_engine = get_db_engine('hub')
    table_name = 'distribution_center_master'
    schema = 'public'
...
def create_verification_table(products_df, master_df):
    """Create verification table for distribution center products"""
    
    print("\n--- Creating Distribution Center Verification Table ---")
    ...
    verification_df.rename(columns={
        ...
        'raw_product_name': 'all_distribution_center_products'
    }, inplace=True)

    # Ensure unique product names in the verification table
    verification_df = verification_df.drop_duplicates(subset=['all_distribution_center_products'])

    try:
        local_engine = get_db_engine('hub')
        
        with local_engine.connect() as conn:
            drop_query = "DROP TABLE IF EXISTS public.distribution_center_verification CASCADE"
            ...
            create_query = """
            CREATE TABLE public.distribution_center_verification (
                id SERIAL PRIMARY KEY,
                all_distribution_center_products TEXT NOT NULL,
                parent_name TEXT,
            ...
            conn.commit()
            print("  ✅ Created distribution_center_verification table")
            
            # Insert data into verification table
            verification_df.to_sql(
                name='distribution_center_verification',
                con=local_engine,
                schema='public',
                if_exists='append',
                index=False
...
def export_verification_to_csv(verification_df):
    ...
    try:
        csv_filename = "distribution_center_verification.csv"
...
def main():
    """Main function"""
    print("Distribution Center Master Table Creation")
    print("=" * 50)
    ...
    try:
        ...
        master_df = create_distribution_center_master_table(products_df)
        if master_df.empty:
        ...
        print(f"\n🎯 DISTRIBUTION CENTER MASTER TABLE CREATED!")
        print(f"📁 Local database: public.distribution_center_master table created")
        print(f"📁 Local database: public.distribution_center_verification table created")
        print(f"📄 CSV file created: distribution_center_verification.csv")
